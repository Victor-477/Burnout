...
#
#  `--save` rewrites ONLY the region between the `bench_vm:begin`
#  and `bench_vm:end` markers in that file. The analysis around it
#  is written by hand and is left alone; see `_write_report`.
#
...
# ── the report, rewritten IN PLACE ───────────────────────────
#
# `--save` used to build the whole file and open it with 'w', so every run
# TRUNCATED the analysis below the table — once, 452 lines down to 34, taking
# the entire 13.4 write-up with it. The safeguard was a paragraph in the file
# asking the reader to copy the prose out first and paste it back, which is a
# safeguard only for as long as everyone reads it.
#
# Only the machine block and the table are generated. Everything else in
# BENCHMARKS.md — the intro, the per-item analysis, the A/B tables that were
# taken by hand — is prose that no run should touch, so the generated region is
# delimited explicitly rather than inferred. Inferring it (find a line starting
# `| Program |`, assume the table runs to the next blank) is the kind of guess
# that works until a second table is added, and this file already has several.
_BEGIN = '<!-- bench_vm:begin — regenerated by --save; edits here are lost -->'
_END = '<!-- bench_vm:end -->'
# Matched by prefix so the wording of the marker can change without stranding
# an existing report.
_BEGIN_KEY = '<!-- bench_vm:begin'
_END_KEY = '<!-- bench_vm:end'


def _generated_block(rows):
    """The volatile part: what this machine measured, on this run."""
...
def _scaffold(rows):
    """A whole report, for when there is not one yet."""
    return [
        "# Pyro VM benchmarks",
        "",
...
def _read_lines(path):
    """The report's lines, plus the line ending it already uses.

    Writing with Python's default translation turns an LF file into CRLF on
    Windows, and then every line of a 493-line report shows as changed. An
    in-place rewrite whose diff is the whole file is barely better than the
    truncating one it replaced — the point is to be able to SEE that only the
    table moved.
    """
...
def _write_report(rows):
    """Replace the generated region, keeping everything around it.

    Returns (ok, message). A report that exists but carries no markers is NOT
    rewritten: truncating it is the exact failure this function was changed to
    prevent, and guessing where the region would have been risks doing it
    silently. Saying so and leaving the file alone is the recoverable outcome.
    """
    block = [_BEGIN] + _generated_block(rows) + [_END]

    if not os.path.isfile(REPORT):
        _write_lines(REPORT, _scaffold(rows), '\n')
        return True, f"written to {REPORT}"

    old, nl = _read_lines(REPORT)

    lo = _find_marker(old, _BEGIN_KEY)
    hi = _find_marker(old, _END_KEY)
    if lo < 0 or hi < 0 or hi < lo:
        missing = ('both markers' if lo < 0 and hi < 0 else
                   f"the {'end' if lo >= 0 else 'begin'} marker")
        return False, (
            f"NOT written: {os.path.basename(REPORT)} is missing {missing}, so "
            f"there is no region to replace.\n"
            f"  The file was left untouched — rewriting it would delete the "
            f"prose, which is what this used to do.\n"
            f"  Put these two lines around the machine block and the table:\n"
            f"    {_BEGIN}\n    {_END}")

    new = old[:lo] + block + old[hi + 1:]
    _write_lines(REPORT, new, nl)
    return True, (f"table rewritten in place in {REPORT} "
                  f"({len(old)} lines in, {len(new)} out)")
