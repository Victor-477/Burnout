...
# ── backend asm: argumentos in the pilha (> nº de registradores) ─
print("[asm] argumentos in the pilha")
SOMA8 = """
...
check("win64 prologo: 8o param in rbp+72", "mov rax, [rbp+72]" in w)
...
check("sysv prologo: 8o param in rbp+24", "mov rax, [rbp+24]" in s)
check("sysv call: reserva 16 (2*8, sem shadow)", "sub rsp, 16" in s)
check("sysv call: reg via r11", "mov rdi, [r11+56]" in s)
check("sysv call: arg de pilha in rsp+0", "mov [rsp+0], rax" in s)
...
""", abi='sysv')   # sysv: 3 args in the pilha -> 24 -> padded 32
check("sysv 3 args pilha -> reserva 32", "sub rsp, 32" in w9)

# ── backend asm: retorno de struct in registrador ───────────
print("[asm] retorno de struct in registrador")
STRUCTS = """
...
"""
# SysV: Point (16B) retorna in RAX:RDX; Boxed (8B) in RAX
...
check("sysv boxed: retorna campo in rax", "boxed:" in s)
check("sysv field access: le p.x de [rbp-16]", "mov rax, [rbp-16]" in s)

# Win64: Boxed (8B) retorna in RAX; Point (16B) -> memoria (Error claro)
WIN_OK = """
struct Boxed { int v; }
...
check("win64 struct 1 campo retorna in rax", "boxed:" in w and "mov [rbp-8], rax" in w)
...
print("[go] tipos and declaracoes")
g = gen_go("""
enum Cor { R, G, B }
...
# divisao INT64_MIN/-1 aborta (como in the runtime C); modulo retorna 0
...
# retorno opcional: value base -> cryoPtr; value ja opcional -> direto
...
print("[fase1] backend C rejeita with Error claro")
...
check("go llm...as T passa o schema", '\\"name\\"' in g)
...
# agent configurável: subconjunto de tools + limite de steps
...
print("[fase3] coerção int/float in aritmética mista")
...
check("pyro sem --sandbox not grava bit2",
...
check("go --sandbox: guarda in the cryoExec",
      'cryoSandboxGuard("pyro_exec")' in _go_sbx)
_go_plain = gen_go('string b = http_get("http://x");')
check("go sem --sandbox: baked false + override by env",
...
check("go: pyro_write_file guardado by sandbox",
      'cryoSandboxGuard("pyro_write_file")' in _go_wf)
check("go sem builtin sensível not emite sandbox",
...
# ── enum + builtins nativos (NATIVE) in the backend pyro ────────
...
# opcodes esperados in the código (usa encode=False p/ ler os bytes in claro)
import disasm_pyro
...
check("bloco >C< with import passa",
...
# library nao qualificada exige import; ambigua with 2 langs
...
check("library qualificada ok with import",
...
check("resolve library by unica importada", _resolve_lib(_Lib(name='math', lang=''), {'c'}) == 'c')
...
print("[auto] seleção de backend by análise de recursos")
...
# capacidades faltando by backend (base da sugestão do --audit)
...
check("miss: concorrencia in pyro -> {concurrency}",
...
# ── Fase 4: módulos + interpolação + const global in the pyro ───
print("[fase4] modulos (import \"file.cryo\")")
...
# parser reconhece a Shape de módulo
ast = ast_of('import "util.cryo" print(double(4));')
check("parser: import \"...\" -> ModuleImport", isinstance(ast.statements[0], _MI))
# resolução: declarações entram, statements executáveis do módulo not
r = resolve_modules(ast, _tmp)
fns = [s.name for s in r.statements if isinstance(s, _FD)]
check("resolve: fn do modulo incorporada", 'double' in fns)
check("resolve: sem ModuleImport in the resultado",
...
# aninhado + dedup (mesmo file by 2 paths)
...
check("resolve: colisao with o programa principal",
      _mod_err('import "util.cryo" fn double(int x) -> int ={ return x; }'))
# codegen ponta-a-ponta with módulo (via compile_source do compiler)
import compiler as _comp
g = _comp.compile_source('import "util.cryo" print(double(21));', 'go', True, base_dir=_tmp)
check("compile_source resolve modulo (go)", "func double(" in g)
...
g = gen_go('print("${1 + 1}");')
check("interp in the inicio ganha contexto string", '"" +' in g.replace("  ", " ") or '""+' in g)
...
print("[fase4] const global in the backend pyro")
...
check("sem: tipo usado in schema_of ok",
...
print("[fase4] try/catch + opcionais in the pyro")
def _pdis(src):
    import disasm_pyro
    return disasm_pyro.disassemble(gen_pyro(src, encode=False))
...
check("pyro const global inlined (CONST, sem LOAD do name)",
...
print("[fase5] input nativo in the pyro")
...
check("fold NAO ocorre in --in the-opt (mantem BOR/BAND)",
...
check("dce: mantem in the --in the-opt",
...
# correção: divisão inteira not é dobrada (semântica de trunc/segurança fica p/ runtime)
...
# a seção de depuração aparece in the disasm with "; linha N"
...
# compilando and rodando (validação de que o offset i32 funciona).
_bigloop = "int s = 0; for (int i = 0; i < 3; i++) { s += i; } print(s);"
check("i32: laço compila and é v2", gen_pyro(_bigloop)[4] == 2)
# --in the-opt not deve conter seção de debug quebrada (round-trip do disasm)
check("debug: disasm --in the-opt ok", "; line" in _dis_noopt(_bigloop))

print("[fase5] HTTP/sleep nativos in the pyro")
...
print("[fase5] JSON nativo in the pyro")
...
check("lsp: dois diagnósticos (aridade + not-declarada)", len(_d) == 2)
check("lsp: diagnóstico traz range/linha 0-based",
      all('range' in x and x['range']['start']['line'] >= 0 for x in _d))
check("lsp: Error léxico vira diagnóstico with coluna",
...
check("lsp: hover de function do usuário", "quad" in (_lsp.hover(_src_ok, 1, 9) or ""))
...
# input já formatada é in the-op
check("fmt: já formatado é in the-op", _fmt.format_source(_fm) == _fm)
...
check("foreign-block ALTO", any(x.rule == 'foreign-block' and x.level == 'HIGH' for x in f))
...
check("tainted-exec ALTO (fonte direta in the sink)",
      ('HIGH', 'tainted-exec') in f)
...
check("taint propaga by atribuicao (a -> b -> sink)",
      ('HIGH', 'tainted-exec') in f)
...
check("hardcoded-secret ALTO (formato de key)",
...
# ── [fase8] funções de 1ª class + lambdas ──────────────────
print("[fase8] funções de 1ª class + lambdas")
...
check("go: tipo de retorno function", "func(int64) int64" in
...
check("pyro: function como value barrada",
...
check("semântica: function como value not acusa 'not declarada'",
      _find(_ast8('fn d(int n)->int ={return n;} '
                  'fn ap(fn(int)->int f)->int ={return f(1);} '
                  'int r = ap(d); print(r);')) == [])

# ── enums with dados & match (etapa 8.2) ──────────────────────
print("[8.2] enums with dados and pattern matching")
...
try:
    ...
    check("semântica: match not exaustivo deveria falhar", False)
except SemanticError:
    check("semântica: match not exaustivo falha (correto)", True)
except Exception as ex:
    check(f"semântica: match not exaustivo falhou with exceção inesperada: {ex}", False)
...
try:
    gen_c(src_adt)
    check("c: enums with dados barrado", False)
except Exception:
    check("c: enums with dados barrado (correto)", True)

try:
    gen_asm(src_adt)
    check("asm: enums with dados barrado", False)
except Exception:
    check("asm: enums with dados barrado (correto)", True)

# ── [8.3] propagação de Error with '?' ────────────────────────
print("[8.3] propagação de Error with '?'")
...
check("parser: ternário not vira propagação",
...
check("go: via de Error retorna cedo", "!__ok { return __try" in g83)
...
# '?' aninhado in outra expressão é barrado with Error claro (go)
try:
...
# c/asm barram a propagação with Error claro
try:
...
check("go: write_bytes guardado by sandbox",
      'cryoSandboxGuard("write_bytes")' in gen_go(_wb))
try:
    from semantic import check as _sem_check
    _sem_check(ast_of(_wb))   # not deve acusar 'function desconhecida'
